...
If you're working on the development version of the package, you can load it
with:
...
We begin by simulating a set of intervention trajectories. Each simulation
starts from a "current" package configuration and stochastically removes and
adds interventions until all candidates have been explored.
...
The resulting tibble records every step in each trajectory, including the
...
head(nodes)
head(dplyr::filter(links, !is.na(next_package)))
```
...
That's all you need to go from stochastic simulations to interactive Sankey
visualisations with **currentsee**. Experiment with alternative weights and
baseline packages to see how the flows evolve, and build on the Shiny app to
create bespoke decision-support tools.
